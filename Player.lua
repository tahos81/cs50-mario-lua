...
function Player:init(map)
    ...
    self.behaviors = {
        ...
        ['walking'] = function(dt)
            ...
            if self.map:wins(self.map:tileAt(self.x + self.width - 1, self.y )) then
                self.sounds['win']:play()
                map:setTile(map.mapWidth - 1, map.mapHeight / 2 - 6, TILE_EMPTY)
                map:setTile(map.mapWidth - 1, map.mapHeight / 2 - 1, FLAG_2)
                love.audio.stop()
                dofile('main.lua')
            end
        end,
        ['jumping'] = function(dt)
            ...
            if self.map:wins(self.map:tileAt(self.x + self.width - 1, self.y + self.height)) then
                self.sounds['win']:play()
                map:setTile(map.mapWidth - 1, map.mapHeight / 2 - 6, TILE_EMPTY)
                map:setTile(map.mapWidth - 1, map.mapHeight / 2 - 1, FLAG_2)
                love.audio.stop()
